...
{
  //standard folders
  QString dir( KStandardDirs::locateLocal( "data", "knode/folders/", false ) );
  if (dir.isNull()) {
//     KNHelper::displayInternalFileError();
  ...
  f = KNFolder::Ptr( new KNFolder( this, 0, i18n( "Local Folders" ), "root" ) );
  mFolderList.append( f );
  f->readInfo();

  f = KNFolder::Ptr( new KNFolder( this, 1, i18n( "Drafts" ), "drafts", root() ) );
  mFolderList.append( f );
  f->readInfo();

  f = KNFolder::Ptr( new KNFolder( this, 2, i18n( "Outbox" ), "outbox", root() ) );
  mFolderList.append( f );
  f->readInfo();

  f = KNFolder::Ptr( new KNFolder( this, 3, i18n( "Sent" ), "sent", root() ) );
  mFolderList.append( f );
  f->readInfo();
...
//     KNHelper::displayInternalFileError();
  ...
  for(QStringList::Iterator it=entries.begin(); it != entries.end(); ++it) {
    f = KNFolder::Ptr( new KNFolder( this ) );
