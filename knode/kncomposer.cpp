...
KNComposer::KNComposer( LocalArticle::Ptr a, const QString &text, const QString &unwraped, bool firstEdit, bool dislikesCopies, bool createCopy, bool allowMail )
...
bool KNComposer::hasValidData()
{
  ...
  if ( a_ctPGPsign->isChecked() ) {
    ...
    NntpAccount::Ptr acc = Akobackit::manager()->accountManager()->account( a_rticle->postingResource() );
    if ( acc ) {
